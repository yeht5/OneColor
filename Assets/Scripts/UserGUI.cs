...
public class UserGUI : MonoBehaviour {
    ...
    void OnGUI()
    {
        if (GUI.Button(new Rect(Screen.width - 200, 150, 40, 40), buttonTexture_Green))
        {
            action.selectColor(color_1);
            colortext.text = "Color : Green";
        }
        if (GUI.Button(new Rect(Screen.width - 200, 225, 40, 40), buttonTexture_Red))
        {
            action.selectColor(color_2);
            colortext.text = "Color : Red";
        }
        if (GUI.Button(new Rect(Screen.width - 200, 300, 40, 40), buttonTexture_Yellow))
        {
            action.selectColor(color_3);
            colortext.text = "Color : Yellow";
        }
        if (GUI.Button(new Rect(Screen.width - 200, 375, 40, 40), buttonTexture_White))
        {
